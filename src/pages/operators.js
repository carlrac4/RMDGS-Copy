...
import {setDocs,addDoc, onSnapshot, snapshot, collection, query, orderBy, where, startAfter, limit, getDocs, doc, endBefore, limitToLast} from "firebase/firestore";
import '../css/driver.css';
import { useNavigate, useParams } from 'react-router-dom';
import {OperatorModal, ConfirmDeleteModal} from './components/operatorModal';
...
import { OperatorMasterList } from "../PrintComponent";
...
function Operator() {
  ...
  const collectionRef = collection(db,"Operator");
...
const showPrevious = ({item}) => {
  ...
  const navigateToConfirmed= (user) =>{
    ...
    return( 
     <div className="min-h-screen flex flex-row " id="operator">
    ...
    <div className="bg-white  border-t-4 border-green-400 shadow rounded-md w-100 md:flex flex justify-center
         no-wrap  mb-3">       
                    <div className=" flex items-center justify-between p-3">
                             <span className=" text-2xl">OPERATOR'S LIST</span>                    
                    </div>
                     <div className=" flex items-center justify-between p-3">
         ...
         no-wrap  mb-3">       
                    <div className=" flex items-center justify-between p-3">
                     ...
                     <div className=" flex items-center justify-between p-3">
                     {/* <OperatorMasterList onClick={unsubscribe} />  */}
                 <ConfirmModal/>
                    </div>
                </div>
                                    ...
                                    {user.map((user) => {
                                      return(
                                        ...
                                        <td className="px-5 py-1 border-b border-black-100 bg-white text-sm">
                                          ...
                                          <td className="px-5 py-1 border-b border-white-600 bg-white text-sm">                                        
                                          <OperatorModal details={{key: user.key, name: user.NAME, body: user.BODY, address: user.ADDRESS }} auth={authentication} />  
                                          {/* <ConfirmDeleteModal details={{key: user.key, name: user.NAME, id: user.NO}} auth={authentication}/> */}
                                            </td>
                                        </tr>
                                        );
                                      })} 
 ...
 function ConfirmModal() {
  ...
  const addLogs = async () => {
    await addDoc(collection(db, "OperatorLogs"), {
      'user':memberID,
      "date":dateEncode,
      'action':"Generated Operator's Masterlist"
      });
  }
  const [showModal, setShowModal] = React.useState(false);
    const  unsubscribe = async () => {
       ...
       const strObj = JSON.stringify(document);  
        localStorage.setItem('OptMasterList', strObj);
      }
    function beforeOpen(){
        if ("OptMasterList" in localStorage) {
            setShowModal(true)
            addLogs();
        } else {
            unsubscribe();  
